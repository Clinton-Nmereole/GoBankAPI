...
<AppShell class="flex flex-row">
		<!-- App Bar -->
		<AppRail class="h-full w-[5%] fixed" background="bg-transparent" >
			<svelte:fragment slot="lead">
                <div class="text-center justify-center items-center" >
                    <AppRailAnchor><button on:click={toggleVisible}><box-icon name='menu' size='sm', color='#CDF0F6'></box-icon></button></AppRailAnchor>
                </div>
			</svelte:fragment>
            {#if visible}
